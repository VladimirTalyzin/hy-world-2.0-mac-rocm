...
# FSDP is only used for multi-GPU sequence-parallel inference. Some PyTorch
# builds ship without the distributed backend at all (e.g. the ROCm wheels for
# Windows, which have no torch._C._distributed_c10d), and importing FSDP there
# raises at module scope. Single-GPU inference does not need it.
try:
...
except (ImportError, ModuleNotFoundError) as _fsdp_exc:  # pragma: no cover
    FSDP = ShardingStrategy = CPUOffload = None
    transformer_auto_wrap_policy = None
    FSDP_IMPORT_ERROR = _fsdp_exc
...
from ..compat import backend as compat
...
def _cast_noncritical_fp32_to_bf16(model, critical_modules):
    # "bf16" in the name is historical: the actual low-precision dtype comes
    # from the backend. MPS has no usable bf16, so compat.preferred_dtype()
    # returns fp16 there; CUDA and ROCm keep bf16.
    lowp = compat.preferred_dtype()
    ...
    for name, param in model.named_parameters():
        if param.dtype == torch.float32 and id(param) not in critical_ids:
            param.data = param.data.to(lowp)
            cast.append(name)
    for _, buf in model.named_buffers():
        if buf.dtype == torch.float32:
            buf.data = buf.data.to(lowp)
    ...
    for name, module in model.named_modules():
        if not any(True for _ in module.children()):
            ...
            if own and all(p.dtype == lowp for _, p in own):
...
def _wrap_model_fsdp(model, sp_group, device, use_cpu_offload=False, enable_bf16=False):
    if FSDP is None:
        raise RuntimeError(
            "Multi-GPU FSDP inference was requested but this PyTorch build has no "
            "distributed support (%s). Run single-GPU instead." % FSDP_IMPORT_ERROR
        )
...
class WorldMirrorPipeline:
    ...
    def from_pretrained(
        ...
        *,
        ...
        if config_path and ckpt_path:
            ...
            del state; gc.collect(); compat.empty_cache()
        else:
        ...
        lowp = compat.preferred_dtype()   # bf16 on CUDA/ROCm, fp16 on MPS
        if enable_bf16:
            if use_fsdp and is_distributed:
                ...
                for _, module in model.named_modules():
                    if not any(True for _ in module.children()):
                        ...
                        if own and all(p.dtype == lowp for p in own):
        ...
        if rank == 0:
            ...
            if compat.device_type() != "cpu":
                alloc = compat.memory_allocated(device) / (1024**3)
    ...
    def __call__(
        ...
        *,
        ...
        # 3. Inference
        if compat.device_type() != "cpu":
            compat.reset_peak_memory_stats(device)
            compat.synchronize(device)
        ...
        if log_time and compat.device_type() != "cpu" and is_distributed:
            peak = compat.max_memory_allocated(device) / (1024**3)
    ...
    def _run_inference(self, img_paths, target_size, prior_cam_path, prior_depth_path):
        ...
        use_amp = compat.device_type() != "cpu" and compat.supports_bf16()
        inner = self.model.module if hasattr(self.model, 'module') else self.model
        model_bf16 = getattr(inner, 'enable_bf16', False)

        t0 = time.perf_counter()
        with compat.autocast(enabled=(not model_bf16 and use_amp)):
...
def main():
    ...
    try:
        ...
        # torch.distributed is a stub module on builds without a distributed
