...
from hyworld2.compat.attention import attention as _portable_attention
...
class Attention(nn.Module):
    ...
    def _apply_attention(self, q: Tensor, k: Tensor, v: Tensor) -> Tensor:
        # Upstream branched here between FlashAttention (which needs a
        # (B, N, H, D) layout) and F.scaled_dot_product_attention. The compat
        # layer keeps the (B, H, N, D) layout the model already uses and picks
        # whichever kernel the backend actually provides -- AOTriton flash on
        # ROCm, the Metal kernels on MPS, FlashAttention-2/3 on CUDA.
        return _portable_attention(
