...
# gsplat is a CUDA extension and has no prebuilt ROCm/Metal wheel. It is only
# needed to *rasterize* splats (training, novel-view rendering, video export);
# feed-forward inference stops at `predictions["splats"]` and never calls it.
# Import it softly so reconstruction works on backends without a gsplat build,
# and fail loudly only if rasterization is actually requested.
try:
    from gsplat.rendering import rasterization
    from gsplat.strategy import DefaultStrategy

    GSPLAT_IMPORT_ERROR = None
except ImportError as _gsplat_exc:  # pragma: no cover - depends on environment
    GSPLAT_IMPORT_ERROR = _gsplat_exc

    class DefaultStrategy:  # placeholder so isinstance()/defaults still resolve
        """Stand-in for gsplat.strategy.DefaultStrategy when gsplat is absent."""

    def rasterization(*args, **kwargs):
        raise ImportError(
            "gsplat is required to rasterize Gaussian splats but is not installed. "
            "Feed-forward reconstruction (points/depth/normals/cameras/splat "
            "parameters) works without it; install gsplat to render images or "
            "export videos. Original error: %s" % GSPLAT_IMPORT_ERROR
        )
