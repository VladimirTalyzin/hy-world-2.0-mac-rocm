...
from hyworld2.compat import backend as compat
...
class WorldMirror(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(
        ...
        """
        if self.enable_bf16:
            views['img'] = views['img'].to(compat.preferred_dtype())
        ...
        with compat.autocast(enabled=(not self.enable_bf16), dtype=torch.float32):
            if sp_size > 1:
    ...
    def extract_priors(self, views):
        ...
        if "camera_poses" in views:
            ...
            poses = cam_params[:, :, :7]
            if self.enable_bf16:
                poses = poses.to(compat.preferred_dtype())
        ...
        if "depthmap" in views:
            ...
            if self.enable_bf16:
                depths = depths.to(compat.preferred_dtype())
        ...
        if "camera_intrs" in views:
            ...
            if self.enable_bf16:
                rays = rays.to(compat.preferred_dtype())
    ...
    def prepare_contexts(self, views, cond_flags, is_inference):
        ...
        if self.enable_bf16:
            context_imgs = context_imgs.to(compat.preferred_dtype())

        with compat.autocast(enabled=(not self.enable_bf16)):
            if use_cond:
                priors = self.extract_priors(views)
                context_priors = (
