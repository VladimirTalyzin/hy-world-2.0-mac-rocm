...
# decord has no Python 3.12 wheels for Windows (see requirements-rocm.txt), and
# it is used for exactly one thing here: random access to a video's last frame.
# OpenCV, already a dependency, can do that.
try:
...
def get_last_video_frame(video_path):
    """Return the final frame of a video as an RGB uint8 array.

    decord supports random access, so index the last frame directly when it is
    installed. Without it, seek with OpenCV and fall back to a sequential scan:
    seeking by frame index is exact for the intra-coded MP4s this pipeline
    writes, but some codecs land on the nearest keyframe and return nothing at
    the very end, in which case reading through is the only correct answer.
    """
    if _HAVE_DECORD:
        vr = VideoReader(video_path, ctx=cpu(0))
        return vr[-1].asnumpy()  # decord seeks internally
