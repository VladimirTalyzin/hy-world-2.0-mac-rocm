...
# This file is meant to be run as a script from inside hyworld2/worldgen, so the
# `hyworld2` package is not on sys.path and the ROCm/MPS compat layer would
# never be applied. Two things depend on it here: the attention backend flag
# (TORCH_ROCM_AOTRITON_ENABLE_EXPERIMENTAL, without which every attention on
# ROCm silently drops to the ~13x slower MATH backend), and the single-process
# torch.distributed stand-ins, without which the first collective call raises
# AttributeError on a build with no c10d backend. Import it before torch is used.
...
if __name__ == '__main__':
    ...
    if _compat.single_process():
        # No c10d backend (ROCm on Windows): a DeviceMesh cannot be built, and a
        # 1x1 one would carry no information. Nothing indexes it unless --fsdp
        # is set, which needs a backend of its own.
        if args.fsdp:
            raise RuntimeError(
                "--fsdp needs a torch.distributed backend, which this PyTorch "
                "build does not have. Run single-GPU without --fsdp."
            )
        device_mesh = None
    else:
