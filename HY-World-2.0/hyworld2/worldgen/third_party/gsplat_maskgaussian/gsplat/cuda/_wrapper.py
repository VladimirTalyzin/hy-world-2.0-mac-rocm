...
def rasterize_to_pixels(
    ...
    packed: bool = False,
    absgrad: bool = False,
    gauss_masks: Optional[Tensor] = None,  # [..., N] or [nnz], per-Gaussian mask (MaskGaussian)
) -> Tuple[Tensor, Tensor]:
    ...
    if gauss_masks is not None:
        gauss_masks = gauss_masks.contiguous()
...
class _RasterizeToPixels(torch.autograd.Function):
    ...
    def forward(
        ...
        gauss_masks: Tensor,  # [..., N] or [nnz], Optional, per-Gaussian mask (MaskGaussian)
        width: int,
        height: int,
        tile_size: int,
        isect_offsets: Tensor,  # [..., tile_height, tile_width]
