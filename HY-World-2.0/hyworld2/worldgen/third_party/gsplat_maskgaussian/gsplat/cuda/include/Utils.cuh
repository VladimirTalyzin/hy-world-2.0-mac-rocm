...
#ifndef USE_ROCM
// HIP has no cooperative_groups/reduce.h; the shims further down replace the
// cg::reduce calls it would provide.
#include <cooperative_groups/reduce.h>
#endif

///////////////////////////////
// ROCm / HIP cooperative-groups shims
///////////////////////////////
//
// HIP's cooperative_groups provides thread_block_tile, tiled_partition,
// binary_partition and coalesced_threads, but NOT cg::reduce and NOT
// cg::labeled_partition. Both are used below and in the backward projection
// kernels. These shims keep the CUDA path byte-identical (everything is behind
// USE_ROCM) while giving HIP a correct equivalent.
#ifdef USE_ROCM

// hipify rewrites <cub/cub.cuh> to <hipcub/hipcub.hpp> but leaves the `cub::`
// call sites untouched, so give them a home.
#include <hipcub/hipcub.hpp>
namespace cub = hipcub;

namespace cooperative_groups {

// Stand-in for labeled_partition(warp, label). The kernels use it only to group
// threads that share a gaussian/camera id so their gradients can be summed once
// and written with a single atomic. A group of one is the degenerate, correct
// case: warpSum becomes a no-op and every thread's thread_rank() is 0, so each
// thread performs its own atomicAdd. Same result, more atomic traffic.
struct hyworld_single_thread_group {
    ...
    __device__ __forceinline__ float shfl_xor(float v, unsigned int) const { return v; }
};

template <class GroupT, class LabelT>
__device__ __forceinline__ hyworld_single_thread_group
labeled_partition(const GroupT &, LabelT) {
    return hyworld_single_thread_group{};
}

} // namespace cooperative_groups

// cg::reduce replacements. Every use in this file is a warp-wide sum or max,
// which a shfl_xor butterfly computes exactly.
template <class WarpT>
__device__ __forceinline__ float hyworld_warp_sum(WarpT &warp, float v) {
    for (unsigned int off = warp.size() >> 1; off > 0u; off >>= 1) {
        v += warp.shfl_xor(v, off);
    }
    return v;
}

template <class WarpT>
__device__ __forceinline__ float hyworld_warp_max(WarpT &warp, float v) {
    for (unsigned int off = warp.size() >> 1; off > 0u; off >>= 1) {
        v = fmaxf(v, warp.shfl_xor(v, off));
    }
    return v;
}

template <class WarpT>
__device__ __forceinline__ int hyworld_warp_max_int(WarpT &warp, int v) {
    for (unsigned int off = warp.size() >> 1; off > 0u; off >>= 1) {
        v = max(v, warp.shfl_xor(v, off));
    }
    return v;
}

#define GSPLAT_WARP_SUM(warp, v) hyworld_warp_sum(warp, v)
#define GSPLAT_WARP_MAX(warp, v) hyworld_warp_max(warp, v)
#define GSPLAT_WARP_MAX_INT(warp, v) hyworld_warp_max_int(warp, v)

#else

#define GSPLAT_WARP_SUM(warp, v) cg::reduce(warp, v, cg::plus<float>())
#define GSPLAT_WARP_MAX(warp, v) cg::reduce(warp, v, cg::greater<float>())
#define GSPLAT_WARP_MAX_INT(warp, v) cg::reduce(warp, v, cg::greater<int>())
...
#pragma unroll
    for (uint32_t i = 0; i < DIM; i++) {
        val[i] = GSPLAT_WARP_SUM(warp, val[i]);
    }
}

template <class WarpT> inline __device__ void warpSum(float &val, WarpT &warp) {
    val = GSPLAT_WARP_SUM(warp, val);
}

template <class WarpT> inline __device__ void warpSum(vec4 &val, WarpT &warp) {
    val.x = GSPLAT_WARP_SUM(warp, val.x);
    val.y = GSPLAT_WARP_SUM(warp, val.y);
    val.z = GSPLAT_WARP_SUM(warp, val.z);
    val.w = GSPLAT_WARP_SUM(warp, val.w);
}
