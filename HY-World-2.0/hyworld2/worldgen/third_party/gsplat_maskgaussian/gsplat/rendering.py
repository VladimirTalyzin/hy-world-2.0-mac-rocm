...
def rasterization(
    ...
    # MaskGaussian: per-Gaussian mask
    gauss_masks: Optional[Tensor] = None,  # [..., N] or [nnz], per-Gaussian mask
    # distortion loss: disparity-space variance with clamp for scale-robust regularization
    distloss: bool = False,
    ...
    if packed:
        ...
        # MaskGaussian: reindex gauss_masks from [N] to [nnz] in packed mode
        if gauss_masks is not None:
            gauss_masks = gauss_masks.view(B, N)[batch_ids, gaussian_ids]  # [nnz]
        ...
        # MaskGaussian: broadcast gauss_masks from [..., N] to [..., C, N] in non-packed mode
        if gauss_masks is not None:
            gauss_masks = torch.broadcast_to(
                gauss_masks[..., None, :], batch_dims + (C, N)
            )  # [..., C, N]
    ...
    if distributed:
        if packed:
            ...
            # MaskGaussian: include gauss_masks in all-to-all communication
            if gauss_masks is not None:
                (gauss_masks,) = all_to_all_tensor_list(
                    world_size,
                    [gauss_masks],
                    cnts,
                    output_splits=collected_splits,
            ...
            if gauss_masks is not None:
                (gauss_masks,) = all_to_all_tensor_list(
                    world_size,
                    [gauss_masks.flatten(0, 1)],
                    splits=[C_i * N for C_i in C_world],
                    output_splits=[C * N_i for N_i in N_world],
                )
                gauss_masks = reshape_view(C, gauss_masks, N_world)

    # Rasterize to pixels
    if render_mode in ["RGB+D", "RGB+ED"]:
        if distloss:
            disps = 1.0 / depths.clamp(min=1e-4)
            colors = torch.cat((colors, depths[..., None], disps[..., None], (disps ** 2)[..., None]), dim=-1)
        else:
            colors = torch.cat((colors, depths[..., None]), dim=-1)
        if backgrounds is not None:
            bg_zeros = [torch.zeros(batch_dims + (C, 1), device=backgrounds.device)]
            if distloss:
                bg_zeros.append(torch.zeros(batch_dims + (C, 1), device=backgrounds.device))
                bg_zeros.append(torch.zeros(batch_dims + (C, 1), device=backgrounds.device))
            backgrounds = torch.cat([backgrounds] + bg_zeros, dim=-1)
    elif render_mode in ["D", "ED"]:
        if distloss:
            disps = 1.0 / depths.clamp(min=1e-4)
            colors = torch.cat((depths[..., None], disps[..., None], (disps ** 2)[..., None]), dim=-1)
        else:
            colors = depths[..., None]
        if backgrounds is not None:
            if distloss:
                backgrounds = torch.zeros(batch_dims + (C, 3), device=backgrounds.device)
            else:
    ...
    if render_mode in ["ED", "RGB+ED"]:
        if distloss:
            render_colors = torch.cat(
                [
                    render_colors[..., :-3],
                    render_colors[..., -3:-2] / render_alphas.clamp(min=1e-10),
                    render_colors[..., -2:-1] / render_alphas.clamp(min=1e-10),
    ...
    if distloss and render_mode in ["D", "ED", "RGB+D", "RGB+ED"]:
        if render_mode in ["ED", "RGB+ED"]:
            disp_expected = render_colors[..., -2:-1]
            disp_sq_expected = render_colors[..., -1:]
        else:
            alpha_safe = render_alphas.clamp(min=1e-10)
            disp_expected = render_colors[..., -2:-1] / alpha_safe
            disp_sq_expected = render_colors[..., -1:] / alpha_safe
        disp_var = (disp_sq_expected - disp_expected ** 2).clamp(min=0, max=1.0)
        meta["render_distort"] = disp_var
        render_colors = render_colors[..., :-2]
