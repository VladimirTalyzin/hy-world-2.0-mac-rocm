...
class DefaultStrategy(Strategy):
    ...
    def step_post_backward(
        ...
        if step % self.reset_every == 0 and step > 0:
            # Anchor protection: backup anchor opacity before reset
            anchor_opa_backup = None
            if state.get("anchor_opa_reset_protection", False) \
               and "is_anchor" in state and state["is_anchor"] is not None:
                anchor_mask = state["is_anchor"]
                if anchor_mask.any():
                    anchor_opa_backup = params["opacities"].data[anchor_mask].clone()

            # Align protection: backup align opacity before reset
            align_opa_backup = None
            if state.get("align_opa_reset_protection", False) \
               and "is_align" in state and state["is_align"] is not None:
                align_mask_r = state["is_align"]
                if align_mask_r.any():
                    align_opa_backup = params["opacities"].data[align_mask_r].clone()
            ...
            # Anchor protection: restore anchor opacity after reset
            if anchor_opa_backup is not None:
                params["opacities"].data[anchor_mask] = anchor_opa_backup

            # Align protection: restore align opacity after reset
            if align_opa_backup is not None:
                params["opacities"].data[align_mask_r] = align_opa_backup
    ...
    def _grow_gs(
        ...
        # Anchor protection: prevent anchor points from being duplicated or split
        anchor_no_densify = state.get("anchor_no_densify", False)
        if anchor_no_densify and "is_anchor" in state and state["is_anchor"] is not None:
            anchor_mask = state["is_anchor"]
            n_anchor_dupli = (is_dupli & anchor_mask).sum().item()
            n_anchor_split = (is_split & anchor_mask).sum().item()
            is_dupli = is_dupli & ~anchor_mask
            is_split = is_split & ~anchor_mask
            if self.verbose and (n_anchor_dupli > 0 or n_anchor_split > 0):
                print(
                    f"[Anchor NoDensify] Blocked {n_anchor_dupli} anchor duplications, "
                    f"{n_anchor_split} anchor splits."
                )

        # Align protection: prevent align points from being duplicated or split
        align_no_densify = state.get("align_no_densify", False)
        if align_no_densify and "is_align" in state and state["is_align"] is not None:
            align_mask = state["is_align"]
            n_align_dupli = (is_dupli & align_mask).sum().item()
            n_align_split = (is_split & align_mask).sum().item()
            is_dupli = is_dupli & ~align_mask
            is_split = is_split & ~align_mask
            if self.verbose and (n_align_dupli > 0 or n_align_split > 0):
                print(
                    f"[Align NoDensify] Blocked {n_align_dupli} align duplications, "
                    f"{n_align_split} align splits."
                )
        ...
        if n_dupli > 0:
            ...
            # New Gaussians from duplication are NOT anchors or align points.
            if "is_anchor" in state and state["is_anchor"] is not None:
                state["is_anchor"][-n_dupli:] = False
            if "is_align" in state and state["is_align"] is not None:
        ...
        if n_split > 0:
            ...
            # Split children are NOT anchors or align points.
            if "is_anchor" in state and state["is_anchor"] is not None:
    ...
    def _prune_gs(
        ...
        # Anchor protection: never prune anchor points during densification
        if "is_anchor" in state and state["is_anchor"] is not None:
            is_prune = is_prune & ~state["is_anchor"]

        # Align protection: never prune align points during densification
        align_no_prune = state.get("align_no_prune", False)
        if align_no_prune and "is_align" in state and state["is_align"] is not None:
            is_prune = is_prune & ~state["is_align"]
