...
# torch.distributed.nn is unavailable on PyTorch builds without a distributed
# backend (the ROCm wheels for Windows have no torch._C._distributed_c10d), and
# importing it here would make `import gsplat` fail outright. Everything in this
# module is multi-GPU only, so a soft import is enough.
try:
    import torch.distributed.nn.functional as distF
except (ImportError, AttributeError, ModuleNotFoundError) as _distF_exc:  # pragma: no cover
    distF = None
    DISTF_IMPORT_ERROR = _distF_exc
